...
namespace jcp {
    ...
    void Provider::addKeyFactoryAlgorithm(const KeyFactoryAlgorithm *algorithm, std::unique_ptr<KeyFactoryFactory> factory)
    {
        kf_algos_.emplace((void*)algorithm, AlgorithmItem<KeyFactoryFactory>(algorithm, factory));
    }
    ...
    void Provider::setKeyUtils(std::unique_ptr<KeyUtils> key_utils)
    {
        key_utils_ = std::move(key_utils);
    }
    ...
    KeyFactoryFactory *Provider::getKeyFactoryFactory(uint32_t algo_id) {
    ...
    KeyUtils* Provider::getKeyUtils() {
        return key_utils_.get();
    }
