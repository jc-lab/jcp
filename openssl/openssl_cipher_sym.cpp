    ...
    namespace openssl {
        ...
        public:
            ...
            std::unique_ptr< Result<void> > updateAAD(const void *auth, size_t length) override {
                ...
                if (rc != 1)
                {
                    return std::unique_ptr< Result<void> >(ResultBuilder<void, exception::InvalidInputException>().withException("Set AAD failed", rc).build());
                }
                return std::unique_ptr<Result<void>>(ResultBuilder<void, void>().build());
            }
            ...
            std::unique_ptr< Result<Buffer> > doFinal() override {
				...
				if (rc != 1) {
					...
					return std::unique_ptr<Result<Buffer>>(ResultBuilder<Buffer, exception::AEADBadTagException>().withException(buf, ossl_eno).build());
				}
